...
function displayJapaneseCars($csvFilePath) {
    ...
    while (($row = fgetcsv($file)) !== FALSE) {
        if ($isFirstRow) {
            ...
            // Assuming CSV structure: Model Name, Manufacturer, Year of Release, Engine Cylinders
            $modelName = $row[0];
            $manufacturer = $row[1];
            $year = $row[2];
            
            // Create search query
            $searchQuery = urlencode($manufacturer . " " . $modelName . " " . $year);
            $googleImageUrl = "https://www.google.com/search?tbm=isch&q=" . $searchQuery;
            
            echo "<td><a href='" . $googleImageUrl . "' target='_blank' style='color: #4CAF50; text-decoration: none;'>🔍 Images</a></td>";
            echo "</tr>";
        }
    }
